...
def f_default_arg_mutable(x: int, data: List[int] = None) -> List[int]:
    if data is None:
        data = []
    if len(data) < 3:
        data.append(x)
...
def f_indexing_and_slicing(data: List[int]) -> Tuple[int, int, List[int]]:
    return data[0], data[len(data)-1], data[0:len(data)-1]
...
def f_str_formatting(array: List[float]) -> str:
    string = "{:.2f}".format(array[0])
    for ele in array[1:]:
        string += (" - {:.2f}".format(ele))
    return string
...
def f_logic(x: int, b1: bool, b2: bool) -> bool:
...
def f_comprehensions(dct: Dict[int, int]) -> Tuple[int, Set[int]]:
    ...
    for ele in range(2, 5):
        x1 += ele
    x1 += 6
    for ele in range(8, 11):
        x1 += ele
    y1 = set([ele for ele in dct.keys() if ele in range(1, 6)])
    return x1, y1
